...
- Update 2026-07-03: Pi job-search history backfill completed. Added `generate-vacancy-backfill-manifest` and `import-vacancy-backfill` to `tools/job-search-runtime/job_search_runtime.py`, generated `automation/state/job-search-backfill-manifest.json` from the local `job-intake/INDEX.md`, and applied it on the Pi canonical checkout. The Pi SQLite runtime now has 231 `backfill:local-personal-office-job-intake` vacancy rows and 262 artifact links, with zero import conflicts and no writes to `processed_messages`. The 189 unique manifest-linked Markdown artifacts were copied to the Pi, and the final audit found 0 missing manifest-linked artifacts plus a repeat dry-run of 231 no-op rows. Evidence: `automation/runs/2026-07-03-job-search-history-backfill.md`.
