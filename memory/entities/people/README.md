# People Entities

Use this folder for stable people records, relationship context, and follow-up anchors.

Source-of-truth people notes remain in `people/`.

## Source Of Truth

- `people/README.md`
- `people/contacts/`
- `people/relationship-notes/`

## Current Important People

| Person | Relationship / role | Source |
|---|---|---|
| Екатерина Резина | wife | `people/contacts/ekaterina-rezina.md` |
| Сергей Резин | son | `people/contacts/sergey-rezin.md` |
| Павел Резин | son | `people/contacts/pavel-rezin.md` |
| Виктория Резина | daughter | `people/contacts/viktoria-rezina.md` |
| Серафима Резина | daughter | `people/contacts/serafima-rezina.md` |
| Артем | Fincom SCADA/СКАДА specialist | `people/contacts/artem-fincom-scada.md` |
| Виктор Геронимус | Fincom partner | `people/contacts/viktor-geronimus.md` |
| Барильник Станислав | AI Studio CTO / Solution Architect | `people/contacts/stanislav-barilnik.md` |
| Барильник Лилия | AI Studio Lead Developer | `people/contacts/lilia-barilnik.md` |
| Дарья Гладышева | MedVoice sales director | `people/contacts/darya-gladysheva.md` |
| Алена Мальцева | RTCloud / Ya-Ty-IT contact | `people/contacts/alena-maltseva.md` |
| Александра Петрова | CNMT technical director | `people/contacts/alexandra-petrova.md` |
| Лариса Титова | Sanatorium Svetly general director | `people/contacts/larisa-titova.md` |
| Yuliya Malinina | Setronica founder and CEO | `people/contacts/yuliya-malinina.md` |

## Privacy

Family birth dates are private family facts. Use them only when necessary for planning, documents, or explicit user requests.
