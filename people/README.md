# People

Люди, контакты, отношения, follow-ups.

## Folders

- `contacts/` - контактные карточки и роли.
- `relationship-notes/` - контекст отношений и история взаимодействий.
- `follow-ups/` - обещания, ожидания и следующие касания.

## Rule

Не хранить чувствительные персональные детали без практической необходимости.

## Contact Index

### Family

- `contacts/ekaterina-rezina.md` - Екатерина Резина, жена.
- `contacts/sergey-rezin.md` - Сергей Резин, сын.
- `contacts/pavel-rezin.md` - Павел Резин, сын.
- `contacts/viktoria-rezina.md` - Виктория Резина, дочь.
- `contacts/serafima-rezina.md` - Серафима Резина, дочь.

### Business / Projects

- `contacts/viktor-geronimus.md` - Виктор Геронимус, партнер по Fincom.
- `contacts/stanislav-barilnik.md` - Барильник Станислав, AI Studio CTO / Solution Architect.
- `contacts/lilia-barilnik.md` - Барильник Лилия, AI Studio Lead Developer.
- `contacts/darya-gladysheva.md` - Дарья Гладышева, директор по продажам MedVoice.
- `contacts/alena-maltseva.md` - Алена Мальцева, RTCloud / Я-Ты-ИТ.
- `contacts/alexandra-petrova.md` - Александра Петрова, технический директор ЦНМТ.
- `contacts/larisa-titova.md` - Лариса Титова, генеральный директор санатория Светлый.
- `contacts/yuliya-malinina.md` - Yuliya Malinina, founder and CEO of Setronica.

### Team Role Notes

- `relationship-notes/medvoice-grant-team-roles.md` - ролевой состав команды MedVoice из грантовых документов.
