...
5. For plain HH/LinkedIn vacancy links sent through Telegram, the only valid control flow is:
   `create/update handoff -> run automation/scripts/enqueue-pi-job-search-handoff.sh <handoff-path> -> reply immediately that the analysis was accepted`.
...
7. Do not wait inside the Telegram turn for job-search to finish. The async worker sends the final follow-up after the dispatcher records the job-search result.
8. If the async enqueue cannot be run, say it is blocked and name the handoff path. Do not send a direct duplicate/no-op conclusion.
9. Reply to the user in concise human language with route, artifact path, and next action.
