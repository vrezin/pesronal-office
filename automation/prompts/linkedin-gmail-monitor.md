...
4. For `status_update`:
   ...
   - if the vacancy belongs on the job-intake workflow, update the relevant per-vacancy task in `tasks/active/` or `tasks/waiting/`;
...
5. For `invitation`:
   ...
   - if the invitation belongs on the job-intake workflow, update the relevant per-vacancy task in `tasks/active/` or `tasks/waiting/`;
...
6. For `new_vacancy`:
   ...
   - update the relevant per-vacancy task in `tasks/active/` or `tasks/waiting/` with the new vacancy state and next step;
...
If repository writes succeed but Git is unavailable, still mark the run successful and advance `automation/state/linkedin-gmail-monitor-state.md`. Mention in the run log that no commit was attempted by policy.
